...
function summarizePaymentChannel(node) {
  ...
  const summary = {
    ...
    // BRT from the channel. This comes from the Destination field of the transaction
    // that created the channel.
    destination: final.Destination,

    // Total BRT, in drops, that has been allocated to this channel.
    // This includes BRT that has been paid to the destination address.
    ...
    // Total BRT, in drops, already paid out by the channel.
    // The difference between this value and the Amount field is how much BRT can still
  ...
  if (prev.Amount) {
    // The change in the number of BRT drops allocated to this channel.
  ...
  if (prev.Balance) {
    // The change in the number of BRT drops already paid out by the channel.
